...
public class Segmenttree2
{
    ...
    public static int query(int tree[], int arr[], int node, int start, int end, int l, int r)
    {
        ...
        if(l<=start && end<=r)
            return tree[node];
        int mid = (start+end)/2;
        int p1 = query(tree,arr,2*node,start,mid,l,r);
        int p2 = query(tree,arr,2*node+1,mid+1,end,l,r);
        return p1 + p2;
    }
    ...
    public static void main(String []args)
    {
        ...
        System.out.println(query(segtree,arr,1,1,n,1,1));
   /*     update(segtree,arr,1,1,n,2,5);
        for(int i=1;i<2*n+2;i++)
            System.out.println(segtree[i]);*/
    }
}
